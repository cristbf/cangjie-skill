...
- 允许引用 **2-4 个片段**,每个片段 ≤150 字
- 每个片段必须标注出处 (章节 / 页码 / 段落标识)
- 片段之间用 `...` 或 `[此处省略论证过程]` 连接
- **总引用量上限 500 字**（理论型书籍放宽到 800 字）
- 若原书是英文,引用英文原文 + 你自己翻译的中文,**不要用现成译本** (避免译者版权 + 译本可能失真)

**为什么改为分段引用**: 单个 150 字上限对多步推理型方法论是灾难 — 压缩过程本身就是信息丢失。分段引用允许保留关键推理节点,同时控制总量。
...
**C 类（理论型）调整**: 理论型 skill 的 E 段不写"操作步骤",写"论证复现步骤" — 即"理解这个论证需要先理解 X,然后按以下路径推理: 前提 → 步骤 1 → 步骤 2 → 结论"。完成标准不是"做了某个动作",而是"能用自己的话复述整个推理路径"。
