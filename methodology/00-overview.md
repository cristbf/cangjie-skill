...
```
          ┌───────────────────┐
          │ 阶段 -1: 类型判断  │  框架型/叙事型/理论型/体验型
          └─────────┬─────────┘
                    │ 调整流水线参数
                    ▼
          ┌───────────────────┐
          │ 阶段 0: 整书理解   │  Adler 四步 + 收集用户真实问题
          └─────────┬─────────┘
                    │ BOOK_OVERVIEW.md
                    ▼
          ┌───────────────────┐
          │ 阶段 1: 并行提取   │  6 个 sub-agent 同时跑
          └─────────┬─────────┘
                    │ candidates/
                    ▼
          ┌───────────────────┐
          │ 阶段 1.5: 三重验证 │  V3基线预处理 → V1跨域 / V2预测力 / V3独特性
          └─────────┬─────────┘
                    │ 通过单元 + rejected/
                    ▼
          ┌───────────────────┐
          │ 阶段 2: RIA++ 构造 │  R(分段引用) / I / A1 / A2 / E / B(+张力)
          └─────────┬─────────┘
                    │ 每个 skill 的 SKILL.md
                    ▼
          ┌───────────────────┐
          ...
          └─────────┬─────────┘
                    ...
                    ▼
          ┌───────────────────┐
          │ 阶段 5: 反馈整合   │  feedback.json + 信号分析 + 迭代改进 (可选)
          └─────────┬─────────┘
                    │
                    ▼
